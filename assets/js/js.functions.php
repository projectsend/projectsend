		...
		if (action != 'none') {
			...
			?>
				...
				if (action == 'log_download') {
					...
					$('.modal_content').html('<p class="loading-img">'+
												'<img src="<?php echo ASSETS_IMG_URL; ?>/ajax-loader.gif" alt="Loading" /></p>'+
			...
			?>
				if (action == 'zip') {
					...
					$('.modal_content').html('<p class="loading-img"><img src="<?php echo ASSETS_IMG_URL; ?>/ajax-loader.gif" alt="Loading" /></p>'+
												'<p class="lead text-center text-info"><?php echo addslashes(__('Please wait while your download is prepared.','cftp_admin')); ?></p>'+
