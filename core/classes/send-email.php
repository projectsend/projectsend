...
$email_template_header = file_get_contents(EMAIL_TEMPLATES_DIR.'/'.EMAIL_TEMPLATE_HEADER);
$email_template_footer = file_get_contents(EMAIL_TEMPLATES_DIR.'/'.EMAIL_TEMPLATE_FOOTER);
...
class PSend_Email
{
	...
	function email_prepare_body($type)
	{
		...
		if ($body_check == '0') {
			$this->get_body = file_get_contents(EMAIL_TEMPLATES_DIR.'/'.$filename);
		}
		else {
