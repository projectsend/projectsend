 ...
 * New version check
 */
if (defined('VERSION_NEW_AUTOCHECK') && (VERSION_NEW_AUTOCHECK === true)) {
				...
				foreach ($feed->channel->item as $item) {
					...
					while ($v < $max_items) {
						...
						if ($online_version > $current_version) {
							 ...
							 * to the database.
							 */
							$statement = $dbh->prepare("UPDATE `" . TABLE_OPTIONS . "` SET value = :version WHERE name='version_new_number'");
							$statement->bindParam(':version', $release->version);
							$statement->execute();
							
							$statement = $dbh->prepare("UPDATE `" . TABLE_OPTIONS . "` SET value = :link WHERE name='version_new_url'");
							$statement->bindParam(':link', $item->link);
							$statement->execute();
							
							$statement = $dbh->prepare("UPDATE `" . TABLE_OPTIONS . "` SET value = :changelog WHERE name='version_new_chlog'");
							$statement->bindParam(':changelog', $release->changelog);
							$statement->execute();
							
							$statement = $dbh->prepare("UPDATE `" . TABLE_OPTIONS . "` SET value = :security WHERE name='version_new_security'");
							$statement->bindParam(':security', $diff->security);
							$statement->execute();
							
							$statement = $dbh->prepare("UPDATE `" . TABLE_OPTIONS . "` SET value = :features WHERE name='version_new_features'");
							$statement->bindParam(':features', $diff->features);
							$statement->execute();
							
							$statement = $dbh->prepare("UPDATE `" . TABLE_OPTIONS . "` SET value = :important WHERE name='version_new_important'");
							$statement->bindParam(':important', $diff->important);
							$statement->execute();
...
if ($last_update !== false) {
	...
	/**
	 * Auto update for authorized users
	 */
