...
class ClientActions extends Base
{
	...
	{
        $validation = new \ProjectSend\Validation();
     ...
     */
    public function generate_username($string, $i = 1)
    {
		$string = preg_replace('/[^A-Za-z0-9]/', "", $string);
		$username = $string;
		while (!$this->is_unique_username($username)) {
			$username = $string . $i;
			$i++;
		}
		return $username;
     ...
     */
    private function is_unique_username($string)
    {
