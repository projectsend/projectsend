...
class CurrentSessionMiddleware implements MiddlewareInterface
{
    protected $router;
    protected $pdo;
    ...
    public function __invoke(Request $request, Response $response, callable $next)
    {
        ...
        if ( !defined( 'IS_INSTALL' ) ) {
             ...
             */
            $this->session = new \ProjectSend\Classes\AccountLoggedIn($this->pdo);
            $global_user = $this->session->get_current_user_username();
            $global_level = $this->session->get_current_user_level();

            /**
             * Get the user information from the database
             */
            $this->account = new \ProjectSend\Classes\Account($this->pdo);
            if ($global_level != 0) {

                $global_account = $this->account->get_user_by_username($global_user);
            }
            else {
                $global_account = $this->account->get_client_by_username($global_user);
            }
