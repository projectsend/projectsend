...
if (557 > LAST_UPDATE) {
    $alter = array();
    $statement = $dbh->exec('ALTER DATABASE ' . DB_NAME . ' CHARACTER SET utf8 COLLATE utf8mb4_general_ci');
    ...
    foreach ( $alter as $key => $value ) {
        $statement = $dbh->prepare("ALTER TABLE $value DEFAULT CHARACTER SET utf8 COLLATE utf8mb4_general_ci");
        $statement->execute();
    }
 ...
 */
if (1097 > LAST_UPDATE) {
    $statement = $dbh->query("ALTER TABLE " . TABLE_USERS . " CHANGE `password` `password` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8mb4_general_ci NOT NULL");
    $updates_made++;
}