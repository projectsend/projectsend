	...
	<?php
		...
		<div class="col-sm-8 col-sm-offset-4">
			<label for="add_client_form_notify">
				<input type="checkbox" name="add_client_form_notify" id="add_client_form_notify" <?php if ($user_form_type == 'new_user') { echo 'checked="checked"' } ?> /> <?php _e('Notify new uploads by e-mail','cftp_admin'); ?>
			</label>
		</div>
	</div>
		<?php
